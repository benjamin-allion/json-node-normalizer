...
const convert = (jsonObject, paths) => {
  ...
  pathsToConvert.forEach((path) => {
    logger.log('debug', "[json-single-to-array] Try to convert '%s' to array", path);
...
const _convert = (jsonObject, path) => {
  logger.log('debug', '[json-single-to-array][convert] Json object conversion method called for %s', path);
  logger.log('debug', '[json-single-to-array][convert] Current JSON to Transform : %j', jsonObject);
  ...
  if (!jsonResult[currentField]) {
    logger.log('debug', "[json-single-to-array][convert] Field doesn't exists : %s", path);
    return jsonResult;
  }
  ...
  if (currentFieldIsArray) {
    logger.log('debug', "[json-single-to-array][convert] Current field to transform '%s' is an array.", currentField);
    const nextPath = path.substr(path.indexOf('*.') + 2);
    jsonResult[currentField].forEach((arrayEntry, index) => {
      logger.log('debug', "[json-single-to-array][convert] Recursive call for '%s[%i]' -> '%s'", currentField, index, nextPath);
  ...
  if (nextField !== null) {
    ...
    logger.log('debug', "[json-single-to-array][convert] Recursive call for '%s' -> '%s'", currentField, nextPath);
    jsonResult[currentField] = convert(jsonResult[currentField], nextPath);
  } else {
    logger.log('debug', '[json-single-to-array][convert] Convert Json Object %s to Array !', currentField);
    jsonResult[currentField] = !Array.isArray(jsonResult[currentField])
      ? [jsonResult[currentField]] : jsonResult[currentField];
  }

  logger.log('debug', '[json-single-to-array][convert] Conversion complete : %j', jsonResult);
  return jsonResult;
};
