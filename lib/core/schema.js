 ...
 */
const getFieldPaths = async (schema) => {
  const fullSchema = await deReferenceSchema(schema);
  let typePaths = jp.paths(fullSchema, '$..[?(@.type && @.type!="object")]');
...
module.exports = {
  deReferenceSchema,
  getAllFieldTypes: getFieldPaths
};
