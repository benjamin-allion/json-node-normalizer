 ...
 * @param {object} customConfig - normalization configuration parameters (see '/config/normalizer.js')
 * @returns {object}
 */
const normalize = async (jsonNode, jsonSchema, customConfig = {}) => {
  ...
  fields.forEach((field) => {
    jsonResult = normalizePaths({
      jsonNode: jsonResult,
      paths: Array.isArray(field.path) ? field.path : [field.path],
      type: field.type,
      format: field.format,
 ...
 * @param {Array<string>} paths - can be an array or unique path/jsonPath value. (Ex: 'root.field.fieldToConvert')
