...
const normalize = async (jsonNode, jsonSchema, customConfig = {}) => {
  ...
  fields.forEach((field) => {
    jsonResult = normalizePath(jsonResult, field.path, field.type, field.format, field.default);
  });
  return jsonResult;
};
...
const setDefaultValueForPath = (jsonNode, path, defaultValue) => {
  const isFieldDefined = !!jp.value(jsonNode, path);
  if (!isFieldDefined) {
    jp.value(jsonNode, path, defaultValue);
  }
};

/**
 * set default value for a given path (array or simple path)
 * @param {object} jsonNode
 * @param {string} path
 * @param {any} defaultValue
 */
const setDefaultValue = (jsonNode, path, defaultValue) => {
  const arraySymbol = '[*]';
  if (path.includes(arraySymbol)) {
    const newPath = path.replace('$.', '');
    const paths = expandArraysPaths({ object: jsonNode, paths: [newPath], arraySymbol });
    paths.forEach(subPath => setDefaultValueForPath(jsonNode, subPath, defaultValue));
  } else {
    setDefaultValueForPath(jsonNode, path, defaultValue);
  }
};
/**
 ...
 * @param defaultValue - target field default value
 * @returns {object}
 */
const normalizePath = (jsonNode, paths, type, format = undefined, defaultValue = undefined) => {
  let jsonResult = { ...jsonNode };
  ...
  pathsArray.forEach((path) => {
    ...
    jsonResult = normalizeNode(jsonResult, jsonPath, type, format, defaultValue);
  });
  return jsonResult;
};
 ...
 * @private
 */
const normalizeNode = (jsonNode, path, type, format = undefined, defaultValue = undefined) => {
  ...
  if (defaultValue) {
    setDefaultValue(jsonResult, path, defaultValue);
  }
